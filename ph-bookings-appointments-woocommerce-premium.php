...
{
	define( 'PH_BOOKINGS_PLUGIN_VERSION', '3.3.2' );
}
...
if ( class_exists( 'PH_Bookings_API_Manager' ) ) {
	...
	$ph_bookings_api_obj 	= new PH_Bookings_API_Manager( __FILE__, $product_id, PH_BOOKINGS_PLUGIN_VERSION, 'plugin', $server_url, $product_title, 'bookings-and-appointments-for-woocommerce');
...
if(Ph_Bookings_Plugin_Active_Check::plugin_active_check( 'woocommerce/woocommerce.php') && !class_exists('phive_booking_initialze_premium') && !Ph_Bookings_Plugin_Active_Check::plugin_active_check('bookings-and-appointments-for-woocommerce/ph-bookings-appointments-woocommerce.php')){
	...
	class phive_booking_initialze_premium {
		...
		public function __construct() {
			...
			include_once ( 'includes/admin/class-ph-migrate-data-to-availability-table.php' );
		...
		public function phive_admin_scripts() {
			...
			if(isset($_GET['page']) && ($_GET['page']=='add-booking' || $_GET['page']=='bookings' || $_GET['page'] == 'ph-booking-calendar'))
			{
				...
				wp_enqueue_script( 'ph_booking_general_script', plugins_url( '/resources/js/ph-booking-general.js', __FILE__ ), array( 'jquery' ) );
			...
			wp_enqueue_script( 'ph_booking_products', plugins_url( '/resources/js/ph-booking-ajax.js', __FILE__ ), array('jquery'), PH_BOOKINGS_PLUGIN_VERSION, true);
		...
		function phive_booking_scripts()
		{
			...
			)
			{
				...
				wp_enqueue_script( 'ph_booking_product', plugins_url( '/resources/js/ph-booking-ajax.js', __FILE__ ), array('jquery'), PH_BOOKINGS_PLUGIN_VERSION, true);
		...
		public function phive_booking_scripts_theme_porto()
		{
			if( function_exists('wp_get_theme') && (wp_get_theme()=='Porto' || wp_get_theme()=='Porto Child')) 
			{
				...
				wp_enqueue_script( 'ph_booking_general_script', plugins_url( '/resources/js/ph-booking-general.js', __FILE__ ), array( 'jquery','porto-theme' ), PH_BOOKINGS_PLUGIN_VERSION, true );
		...
		function plugin_action_links( $links ) {
			...
			$plugin_links = array(
				...
				'<a href="'.admin_url('post-new.php?post_type=product').'" >' . __('Add Product', 'bookings-and-appointments-for-woocommerce') . '</a>',
			);
